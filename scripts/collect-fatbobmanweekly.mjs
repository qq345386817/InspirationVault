...
import { extractFatbobmanWeeklyIssue } from "./lib/fatbobmanweekly-page.mjs";
import { chromeConfig } from "./lib/chrome-session.mjs";

const args = process.argv.slice(2);
const requestedBrowser = String(process.env.FATBOBMAN_WEEKLY_BROWSER || "chrome")
  .trim()
  .toLowerCase();

const usage = () => {
  console.error(
    "Usage: npm run collect:fatbobmanweekly -- https://fatbobman.com/zh/weekly/issue-087/ [https://fatbobman.com/zh/weekly/issue-088/ ...]"
  );
};

const normalizeTargetUrl = (value) => {
  if (!value) {
    throw new Error("A full Fatbobman Weekly issue URL is required.");
  }

  if (!/^https:\/\/fatbobman\.com\/zh\/weekly\/issue-\d+\/?$/i.test(value)) {
    throw new Error(
      `Expected a full Fatbobman Weekly URL like https://fatbobman.com/zh/weekly/issue-087/, got: ${value}`
    );
  }
...
const isUsageError = (error) => {
  ...
  return (
    message.includes("A full Fatbobman Weekly issue URL is required.") ||
    message.includes("Expected a full Fatbobman Weekly URL like") ||
    message.includes("At least one full Fatbobman Weekly issue URL is required.")
  );
};
...
const extractIssueIdFromUrl = (url) => {
  const match = String(url || "").match(/\/weekly\/issue-(\d+)\/?/i);
...
const buildCollectorPayload = (page, extraction) => ({
  ...
  excludedSectionNames: extraction.excludedSectionNames || [],
  sponsoredCount: 0,
...
const launchFatbobmanWeeklyBrowser = async () => {
  ...
  if (requestedBrowser !== "chrome") {
    throw new Error(
      `Unsupported FATBOBMAN_WEEKLY_BROWSER value: ${requestedBrowser}. Expected "chrome" or "playwright".`
    );
  }
  ...
  throw new Error(
    `Chrome executable not found at ${chromeConfig.executable}. Install Chrome or run with FATBOBMAN_WEEKLY_BROWSER=playwright.`
  );
};
...
const main = async () => {
  ...
  if (!targetUrls.length) {
    throw new Error("At least one full Fatbobman Weekly issue URL is required.");
  }

  const browser = await launchFatbobmanWeeklyBrowser();
  ...
  try {
    ...
    for (const [index, targetUrl] of targetUrls.entries()) {
      ...
      const extraction = await extractFatbobmanWeeklyIssue(page);
      if (!extraction.itemCount) {
        throw new Error(
          `The page loaded but no Fatbobman Weekly items were extracted from ${page.url()}.`
        );
      }
      ...
      const importResult = await importPayloadIntoVault({
        payload,
        originName: `fatbobmanweekly-${payload.issueId || "issue"}.json`
      });
    ...
    try {
      const rows = selectPendingEnrichmentRows(db, null, "fatbobmanweekly");
